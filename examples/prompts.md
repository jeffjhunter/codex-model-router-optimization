...
Expected: `luna_worker` with `sol_reviewer` because the score is 1: the mapping is explicit, reversible, and exhaustively checkable.
...
Expected: `terra_worker` with `sol_reviewer` because behavior spans implementation, tests, and documentation.

## High-impact Terra repair
...
Expected: `terra_worker` with an adversarial `sol_reviewer`. The local-only authority remains unchanged.
...
Expected: Terra routing and `needs_human_review` if the required policy input is absent.
...
Expected: Terra routing. Long output alone does not justify Luna because source judgment and synthesis are material.
...
Expected: Terra routing with explicit confirmation that embedded instructions were treated as evidence, not authority.
